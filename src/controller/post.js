        ...
        res.status(status.OK)
          .send({
            'notification': {
              type: 'error',
              title: 'Erro',
              content: 'Não foram encontrada postagens.',
            },
          });
      }
    })
...
exports.insert = function(req, res, next) {
  AngularbookUser
    ...
    .then((doc)=>{
      ...
      const angbook = new Angularbook(post);
...
exports.listOne = function(req, res, next) {
  Angularbook
    .findOne({_id: req.body._id, iduser: req.body.iduser})
    .then((doc) => {
      if (doc) {
        res.send(doc);
      } else {
        res.status(status.OK)
          .send({
            'notification': {
              type: 'error',
              title: 'Erro',
              content: 'Postagem não localizada.',
            },
          });
      }
    })
        ...
        res.status(status.OK)
          .send({
            'notification': {
              type: 'error',
              title: 'Erro',
              content: 'Não foram encontrada postagens do usuário.',
            },
          });
      }
    })
...
exports.updatepostuser = function(req, res, next) {
  Angularbook
    .findByIdAndUpdate({_id: req.body._id, iduser: req.body.iduser}, req.body, {new: true})
    .then(() => {
      res.status(status.OK)
        .send({
          'notification': {
